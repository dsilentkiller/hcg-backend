				...
				<div class="col-md-12 col-lg-12">
					<div class="card card-info">
						<div class="card-header">
							<div class="card-title">Info of service: {{$service->name}}</div>
						</div>
						<div class="card-body">
							<div class="row">
								<img src="{{asset('uploads/service/thumbnail/'.$service->thumbnail)}}" width="358" height="239">
							</div>
							<hr>
							<div class="form-group mt-2">
								<label>Created Or Updated By: {{$service->user->name}}</label>
							</div>
							{{-- <div class="form-group">
								<label>Duration: {{$service->duration_days}} Days {{$service->duration_nights}} Nights</label><br>
							</div>
							<div class="form-group">
								<label>Difficulty:{{$service->difficulty}}</label><br>
							</div>
							<div class="form-group">
								<label>Start From: $ {{$service->start_from}}</label><br>
							</div> --}}
							<div class="form-group">
							...
							<div class="form-group">
								<label>Title Tag: {{$service->title_tag}}</label>
							</div>
							<div class="form-group">
                            ...
                            @if (isset($thumbnails) &&count($thumbnails)>0)
                                <div class="row">
                                    @foreach($service->thumbnails as $thumnail)
                                    <a href="{{asset('uploads/service/thumbnail/'.$thumnail->image)}}"><img src="{{asset('uploads/service/thumbnail/'.$thumnail->image)}}" height="100" width="100" class="m-2"></a>
                                    @endforeach
                                </div>
                            @endif
						</div>
						<div class="card-footer">
							<div>
								<center><a href="{{route('service.index')}}"><< Go Back</a></center>
							</div>
						</div>
					</div>
				</div>
