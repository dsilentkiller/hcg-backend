							...
							<form class="form-inline" action="{{route('banner.update', $banner)}}" method="post" enctype="multipart/form-data">
								...
								<form class="form-inline" action="{{route('banner.store')}}" method="post" enctype="multipart/form-data">
									...
									<div class="form-group m-2">
										...
										<input type="text" name="title" class="form-control" value="{{old('title', @$banner->title)}}" placeholder="Enter title">
										@error('title')
										<span>{{$message}}</span>
										@enderror
									</div>
                                  {{-- slug --}}
                                  <div class="form-group m-2">
                                    <label for="staticEmail2" class="sr-only">Slug</label>
                                    <input type="text" name="title" class="form-control" value="{{old('slug', @$banner->slug)}}" placeholder="Enter slug">
                                    @error('slug')
                                    <span>{{$message}}</span>
                                    @enderror
                                </div>
